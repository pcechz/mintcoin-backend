...
# Build only this app (define script in package.json, e.g. "build:fraud-service")
# Option 1: nest build directly
# RUN npx nest build fraud-service
# Option 2: via npm script:
# RUN npm run build:fraud-service
RUN npm run build:fraud-service

CMD ["node", "dist/apps/fraud-service/main.js"]
