...
# Build only this app (define script in package.json, e.g. "build:gift-service")
# Option 1: nest build directly
# RUN npx nest build gift-service
# Option 2: via npm script:
# RUN npm run build:gift-service
RUN npm run build:gift-service

CMD ["node", "dist/apps/gift-service/main.js"]
